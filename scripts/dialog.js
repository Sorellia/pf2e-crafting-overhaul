...
import { CheckFeat, getPreferredPaymentMethod, localise, MODULE_NAME } from "./constants.js";

/**
 * Generates a form group HTML for choosing a paying method (for beginning or crafting projets).
 * 
 * @param {"fullCoin" | "preferCoin" | "preferReagent" | "fullReagent" | "free"} preferredDefault One of the potential pay methods.
        ...
        content: `
                <form>
                    <body>
                        ...
                        <section>
                            ${localise("ProjectManagement.MaximumCost")}: <span id="spanNotOverspending"><strong id="maxCost">0 gp</strong></span><span id="spanOverspending" hidden><strong style="color: red">${localise("ProjectManagement.OverspendingWarning")}</strong></span>
                        </section>
                    </body>
        ...
        render: ([content]) => {
            content
                ...
                .addEventListener("keyup", (event) => {
					...
					const spendingDelta = maxCost.copperValue - currentSpending.copperValue;
 ...
 */
export async function projectCraftDialog(actor, itemDetails, progress) {
    const item = await fromUuid(itemDetails.UUID);
    const extraHTML = [];
    let modifiers = [];
    ...
    return await Dialog.wait({
        ...
        <form>
                    ...
                    ${await getPaymentOptionHTML(getPreferredPaymentMethod(actor))}
                    ${extraHTML.join('\n')}
                </form>
        `,
        buttons: {
        ...
        render: ([content]) => {
            content
                ...
                .addEventListener("keyup", (event) => {
                    ...
                    const currentSpending = game.pf2e.Coins.fromString(event.target.value);
					const modifiedSpending = subtractCoins(maxCost, progress);
                    const remainingSpending = subtractCoins(modifiedSpending, currentSpending);
					const spendingDelta = modifiedSpending.copperValue - currentSpending.copperValue;
            ...
            const maxCost = game.pf2e.Coins.fromPrice(item.price, itemDetails.batchSize || 1);
			const remainingCost = subtractCoins(maxCost, progress);
            content
                .querySelector("[id=maxCost]").innerHTML = maxCost;
            content
                .querySelector("[id=remainingMaterials]").innerHTML = remainingCost;
        },
    });
}
...
export async function projectToChat(actor, projectUUID) {
    if (!projectUUID || projectUUID === "") {
        console.error("[CCO]: Missing Project UUID when posting to chat!");
        return;
    }
